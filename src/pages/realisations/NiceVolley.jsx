...
export default function NiceVolley() {
  ...
  const referentiels = [3, 4, 5]
  const pourQuiProofs = getProjectProofs('niceVolley', 'pourQui')
  const pourQuoiProofs = getProjectProofs('niceVolley', 'pourQuoi')
  const commentProofs = getProjectProofs('niceVolley', 'comment')
  const ceQueJaiFailProofs = getProjectProofs('niceVolley', 'ceQueJaiFait')
      ...
      <section className="py-5 px-4 text-center border-bottom hero-section hero-section--small">
        <div style={{ maxWidth: '900px' }}>
          ...
          <h1 className="responsive-title responsive-title--medium">
            CRM pour Nice Volley Ball Club
          </h1>
        </div>
      </section>
